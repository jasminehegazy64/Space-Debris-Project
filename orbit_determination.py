...
fits_filenames = [
    "Copy of NEOS_SCI_2024001001404.fits",
    "Copy of NEOS_SCI_2024001001121.fits",
    "Copy of NEOS_SCI_2024001000838.fits",
    "Copy of NEOS_SCI_2024001000555.fits",
    "Copy of NEOS_SCI_2024001000312.fits",
]
...
def calculate_satellite_position(
    frame, datetime_obs, EPOS_X, EPOS_Y, EPOS_Z, AVG_VEL, RA_VEL, DEC_VEL, ROL_VEL
):
...
def update(frame):
    ...
    x_satellite, y_satellite, z_satellite = calculate_satellite_position(
        frame,
        df_one["Date time"][frame],
        df_one["EPOS_X"][frame],
        df_one["EPOS_Y"][frame],
        df_one["EPOS_Z"][frame],
        df_one["AVG_VEL"][frame],
        df_one["RA_VEL"][frame],
        df_one["DEC_VEL"][frame],
        df_one["ROL_VEL"][frame],
    )
...
for fits_filename in os.listdir(fits_directory):
    if fits_filename.endswith(".fits"):
        ...
        with fits.open(full_path_fits) as hdul:
            ...
            time = Time(
                header["DATE-OBS"], format="fits", scale="utc"
            )  # Assuming time is in FITS format
...
def objective_function(params):
    ...
    for i, time in enumerate(times):
        ...
        residuals.append(
            positions[i].represent_as("cartesian").xyz.value
            - orbit.propagate(time).r.value
        )
...
final_orbit = Orbit.from_classical(
    Earth,
    a_opt * u.km,
    ecc_opt * u.one,
    inc_opt * u.deg,
...
def update(frame):
    ...
    x, y, z = calculate_satellite_position(
        theta_values[frame], inclination, eccentricity, semi_major_axis
    )
...
def update(frame):
    ...
    )  # Assuming radius of Earth circle as 1
