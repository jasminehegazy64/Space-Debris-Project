...
class DebrisAnalyzer:
    ...
    def process_images(self):
        fitsfiles = os.listdir(self.threshed_directory)
        object_data = []  # List to store object data
        ...
        for fits_filename in fitsfiles:
            ...
            for label in range(1, num_labels_iterative):
                ...
                object_data.append([fits_filename, object_id, area_iterative, edge_count, center_x, center_y, w, h, lbp_mean, lbp_std, prediction])
    ...
    def get_total_objects(self, object_data):
        return len(object_data)

    def write_to_csv(self, object_data):
        with open(self.csv_file_path, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(['Image', 'Object ID','Area','Edges','Center_x','Center_y','Width','Height','lbp_mean','lbp_std' ,'Prediction'])
            csvwriter.writerows(object_data)