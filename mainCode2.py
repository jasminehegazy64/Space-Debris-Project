...
Original file is located at
    https://colab.research.google.com/drive/12UQgPyPEhhNcjA4ey0HMwlmxKAkicjQH
"""
...
def draw_boxes(image, objects):
    for obj in objects:
        x, y, w, h = obj.bbox
        cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv2.putText(image, str(obj.id), (x, y - 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    if fits_filename.endswith('.fits'):
        ...
        for i in range(1, num_labels):
            ...
            component_mask = (labels == i).astype(
                np.uint8)  # Mask for the current object
            _, edges = cv2.threshold(cv2.Canny(
                component_mask, 30, 100), 0, 255, cv2.THRESH_BINARY)  # Edges within the object
            corners = cv2.goodFeaturesToTrack(
                component_mask, maxCorners=100, qualityLevel=0.01, minDistance=0.1)  # Corners within the object
            num_corners = corners.shape[0] if corners is not None else 0
            ...
            objects.append(DetectedObject(obj_id, (x, y, w, h),
                           area, np.count_nonzero(edges), num_corners))
            id_mapping[i] = obj_id
        ...
        # Display the result
        cv2_imshow(thresholded_img_vis)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
for fits_filename in os.listdir(fits_directory):
    if fits_filename.endswith('.fits'):
        ...
        image = cv2.normalize(data, None, 0, 255,
    ...
    for fits_filename in os.listdir(fits_directory):
        if fits_filename.endswith('.fits'):
            ...
            for label in range(1, num_labels_iterative):
                ...
                csvwriter.writerow([fits_filename, object_id - 1, area_iterative, edge_count, center_x,
                                   center_y, w, h, lbp_mean, lbp_std, 'Debris' if finalint > 10 else 'Celestial Object'])
...
df[['Area', 'Edges', 'Center_x', 'Center_y', 'Width', 'Height', 'lbp_mean', 'lbp_std']] = scaler.fit_transform(
    df[['Area', 'Edges', 'Center_x', 'Center_y', 'Width', 'Height', 'lbp_mean', 'lbp_std']])
...
def block_matching_optical_flow(prev_frame, next_frame, bounding_boxes, block_size, search_area):
    flow_vectors = []
...
def draw_flow(img, flow, bounding_boxes, step=16):
    ...
    for i, bbox in enumerate(bounding_boxes):
        ...
        x_mid = x + w // 2
        y_mid = y + h // 2
        cv2.arrowedLine(vis, (x_mid, y_mid),
                        (x_mid + dx, y_mid + dy), (0, 255, 0), 1)
    return vis
...
for fits_filename in os.listdir(fits_directory):
    if fits_filename.endswith('.fits'):
        ...
        for i in range(1, num_labels):
            ...
            edges = cv2.Canny(component_mask, 30, 100).sum() / \
                255  # Edge count
            corners = cv2.goodFeaturesToTrack(
            ...
            objects.append(DetectedObject(obj_counter, (x, y, w, h),
                           area, edges, num_corners, center, width, height))
            obj_counter += 1
